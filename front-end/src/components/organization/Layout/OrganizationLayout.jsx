...
const OrganizationLayout = ({ children, title, subtitle, actions }) => {
  ...
  const menuItems = [
    { label: 'Dashboard', icon: '📊', path: ROUTES.ORG_DASHBOARD },
    { label: 'Evaluations', icon: '📝', path: ROUTES.ORG_EVALUATIONS },
    { label: 'Results', icon: '📈', path: ROUTES.ORG_RESULTS },
    { label: 'Settings', icon: '⚙️', path: ROUTES.ORG_SETTINGS },
  ];
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col">
      ...
      <header className="bg-white border-b border-gray-200 sticky top-0 z-40 shadow-sm">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          ...
          >
            <div className="w-10 h-10 bg-green-600 rounded-lg flex items-center justify-center text-xl text-white">
              🏢
            </div>
          ...
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-2 px-3 py-2 bg-gray-100 rounded-lg text-sm font-medium text-gray-700">
              <span>👤</span>
              <span>{user.fullName || user.email || 'Organization'}</span>
            </div>
            <button
