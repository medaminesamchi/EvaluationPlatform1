...
const EvaluatorLayout = ({ children, title, subtitle, actions }) => {
  ...
  const menuItems = [
    { label: 'Dashboard', icon: '📊', path: ROUTES.EVAL_DASHBOARD },
    { label: 'Pending', icon: '⏳', path: ROUTES.EVAL_PENDING },
    { label: 'Reviewed', icon: '✅', path: ROUTES.EVAL_REVIEWED },
  ];
      ...
      <header className="bg-white border-b border-gray-200 sticky top-0 z-40 shadow-sm">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          <div
            ...
            onClick={() => navigate(ROUTES.EVAL_DASHBOARD)}
          >
            <div className="w-10 h-10 bg-yellow-600 rounded-lg flex items-center justify-center text-xl text-white">
              🧑‍⚖️
            </div>
          ...
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-2 px-3 py-2 bg-gray-100 rounded-lg text-sm font-medium text-gray-700">
              <span>👤</span>
              <span>{user.fullName || user.email || 'Evaluator'}</span>
            </div>
            <button
